...
def train(args, model, train_loader, eval_loader, num_epochs, output, opt=None, s_epoch=0):
    ...
    saving_epoch = num_epochs - 1    # Start point for model saving
    ...
    logger.info('optim: adamax lr=%.4f, decay_step=%d, decay_rate=%.2f, grad_clip=%.2f' % (lr_default, lr_decay_step, lr_decay_rate, grad_clip))
    ...
    for epoch in range(s_epoch, num_epochs):
        ...
        if epoch < len(gradual_warmup_steps):
            ...
            logger.info('gradual warm up lr: %.4f' % trainer.optimizer.param_groups[0]['lr'])
        elif epoch in lr_decay_epochs:
            trainer.optimizer.param_groups[0]['lr'] *= lr_decay_rate
            logger.info('decreased lr: %.4f' % trainer.optimizer.param_groups[0]['lr'])
        else:
            logger.info('lr: %.4f' % trainer.optimizer.param_groups[0]['lr'])
            ...
            for i, (v, q, a, _, _, _) in enumerate(train_loader):
                ...
                if i < num_batches - 1 and (i + 1) % update_freq > 0:
                    ...
                    if num_updates % int(args.print_interval / update_freq) == 0:
                        logger.info("Iter: {}, Loss {:.4f}, Norm: {:.4f}, Total norm: {:.4f}, Num updates: {}, Wall time: {:.2f}, ETA: {}".format(i + 1, total_loss / ((num_updates + 1)), grad_norm, total_norm, num_updates, time.time() - wall_time_start, utils.time_since(t, i / num_batches)))
        ...
        logger.info('\ttrain_loss: %.2f, norm: %.4f, score: %.2f' % (total_loss, total_norm / count_norm, train_score))
        if eval_loader is not None:
